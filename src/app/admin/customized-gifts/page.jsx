...
const Gifts = () => {
    ...
    useEffect(() => {
        const handleAuthStateChange = () => {
            ...
            const unsubscribe = auth.onAuthStateChanged(async (user) => {
                if (user) {
                    try {
                        const {data} = await axios.get("/api/customized_gifts/getall" ,{
                                params: {
                                    id: user.uid
                                }
                        });
                        console.log(data.result)
                        let giftData=data.result
                        let OrdersList = giftData.map((order) => {
                            return {
                                ...order,
                                orderId: "#" + order.id,
    ...
    useEffect(()=>{
        ...
        setTotalPages(Math.ceil(displayedOrders.length / giftsPerPage))
  ...
  return (<>
   {displayedOrders.length>0 ? <section className="bg-softBeige flex w-full items-center p-16 antialiased h-[100vh] md:py-16">
      <div className="mx-auto flex-1 max-w-screen-xl px-4 p-16 2xl:px-0 bg-white rounded-3xl">
        <div className="mx-auto max-w-5xl">
          <div className="gap-4 sm:flex sm:items-center sm:justify-between">
            <h2 className="text-xl font-semibold text-gray-900 sm:text-2xl">My Customized Gifts</h2>
          ...
          <div className="mt-6 flow-root sm:mt-8">
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {currentOrders.map((order, index) => (
                <OrderCard
                  url={"/admin/customized-gifts/"+order.orderId.slice(1)}
                  key={index}
                  id={order.orderId}
