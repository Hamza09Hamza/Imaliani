...
import { deleteProduct, getProductsperID, updateProduct } from '@/Firebase/CRUD/Products';
...
function EditPage({ params }: { params: { slug: string } }) {
            ...
            if (slug) {
                try {
                    ...
                    if (data) {
                        ...
                        setFileslinks(data.images.map((img:any)=>({url:img,file:null})))
                    }
                } catch (error) {
    ...
    const handleDeleteImage = (link: string) => {
        const updatedImages = fileslinks.filter((image) => image.url !== link );
        setFileslinks(updatedImages)
        const formedData =updatedImages.map((image)=>image.file||image.url)
        setFormData({ ...formData, images: formedData });
        console.log('Images after deletion:', formedData);
    };
    ...
    const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setFileslinks((prev)=>[...prev,...newFileURLs])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (Object.keys(updatedFields).length > 0) {
                ...
                await updateProduct(slug, { ...updatedFields, images: formData.images });
            }
    ...
    const handleDeleteProduct =async ()=>{
        try {
            ...
            product?.id && await deleteProduct(product.id)
            product?.id && alert('Product updated successfully!');
            product?.id && window.location.assign("/admin/products")
    ...
    return (
        <>
        <div className='bg-softBeige w-[100vw]  flex items-center justify-center pb-40'>
            ...
            <div className=" p-4 bg-white rounded-lg shadow sm:p-5">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid gap-4 mb-4 sm:grid-cols-2">
                        ...
                        <div className='flex xxs:flex-col xxs:items-center xxs:gap-8 justify-around sm:col-span-2 w-[100%] my-4 flex-wrap'>
                            {fileslinks.map((image: any) => (<>
                           
                                    <div className='relative xxs:flex xxs:justify-center   xxs:w-[100%] w-[32%]'>
                                        <Image
                                            src={image.url}
                                            alt={"Product image"}
                                            className='xxs:w-[10rem] w-[100%] rounded-lg '
                                            width={100}
                                            height={100}
                                        />
                                        <button
                                            ...
                                            className="absolute z-10  top-0 right-0 xxs:right-[30%]  text-white rounded-full "
                                        >
                            ...
                            {fileslinks.length < 3 && (
                                <label htmlFor="dropzone-file" className="flex  xxs:w-[90%]  flex-col items-center justify-center w-[32%] h-60 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <button 
                            onClick={async()=>await handleDeleteProduct()}
                            type="button" 
